...
export default function HumanGame() {
  ...
  const handleSquareClick = (row: number, col: number) => {
    ...
    if (!selectedSquare) {
      ...
      if (movingPiece) {
        // Simple move validation (basic - just allow any move for now)
  ...
  const renderPiece = (piece: Piece | null) => {
    ...
    const PieceComponent = ChessPieces[piece.type.charAt(0).toUpperCase() + piece.type.slice(1) as keyof typeof ChessPieces];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="px-4 py-2 bg-white/10 backdrop-blur-xl rounded-xl border border-white/20 text-white hover:bg-white/20 transition-all"
          >
            Exit Game
          </button>
        </div>
